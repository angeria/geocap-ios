...
class AuthViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(AuthViewController.keyboardDidChange),
...
//        }
    ...
    @objc private func keyboardDidChange(notification: Notification) {
        ...
        if endFrame.height != 0, view.frame.height == endFrame.height + endFrame.origin.y {
            ...
            bottomConstraint.constant = view.frame.height - endFrame.origin.y - view.safeAreaInsets.bottom
                                        + buttonToTextFieldConstraint.constant
            let topToEmailTextField = endFrame.origin.y - view.safeAreaInsets.bottom
                                        - buttonToTextFieldConstraint.constant - continueButtonHeightConstraint.constant
                                        - buttonToTextFieldConstraint.constant - emailTextField.frame.height
            iconToTopConstraint.constant = (topToEmailTextField - view.safeAreaInsets.top
    ...
    private func presentConfirmEmailAlert() {
        ...
        let message = NSLocalizedString("auth-email-confirmation-message",
                                        comment: "Message of email confirmation alert")
    ...
    private func handleError(_ error: NSError) -> String? {
        ...
        case .invalidEmail:
            ...
            errorMessage = NSLocalizedString("auth-email-text-field-info-label-invalid-email",
                                             comment: "Text shown when the user inputs an invalid email")
        case .invalidActionCode:
            errorMessage = NSLocalizedString("auth-email-text-field-info-label-invalid-action-code",
                                        comment: "Error message when the user tries to use an old email sign in link")
        default:
            Crashlytics.sharedInstance().recordError(error)
            errorMessage = NSLocalizedString("auth-email-text-field-info-label-error",
                                        comment: "Text shown when something went wrong with using the inputed email")
        }
    ...
    func signInWithLink(_ link: String) {
        ...
        Auth.auth().signIn(withEmail: email, link: link) { [weak self] authResult, error in
            ...
            Crashlytics.sharedInstance()
                .setUserName(UserDefaults.standard.string(forKey: GeoCapConstants.UserDefaultsKeys.username))
    ...
    private func setUsername(forUser user: User) {
        ...
        profileChangeRequest.displayName = UserDefaults.standard.string(forKey:
            GeoCapConstants.UserDefaultsKeys.username)
